...
out_unlock:
  task_unlock(
      prev); /* Synchronise here with release_task() if prev is TASK_ZOMBIE */
  return;
...
extern int phase2switchflag;
unsigned long user_run_counts[65536];
...
need_resched_back:
  ...
  /* Alternating User Scheduler preemption rotation */
  if (phase2switchflag == 2 && prev->state == TASK_RUNNING &&
...
repeat_schedule:
  ...
  } else if (phase2switchflag == 2) {
    unsigned long min_runs = 0xffffffff;
    struct task_struct *candidate = NULL;
    next = idle_task(this_cpu);

    /* 1. Kuyruktaki süreçlerden, kullanıcısı en az çalışmış olanı bul */
    list_for_each(tmp, &runqueue_head) {
      p = list_entry(tmp, struct task_struct, run_list);
      if (can_schedule(p, this_cpu) && user_run_counts[p->uid] < min_runs) {
        min_runs = user_run_counts[p->uid];
        candidate = p;
      }
    }

    /* 2. Seçilen süreci ata ve o kullanıcının sayacını 1 artır */
    if (candidate) {
      next = candidate;
      user_run_counts[candidate->uid]++;
      c = 1000;
    } else {
      c = 0;
    }
